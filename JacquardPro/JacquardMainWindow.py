...
class JacquardMainWindow(Ui_MainWindow):
    ...
    def __init__(self):
        ...
        if self.jacquard_tester is None:
            ...
            self.jacquard_tester.signal_happen.connect(self.__signal_happen)
    ...
    def _start_device_moniter(self):
        if self.jacquard_tester.get_monitor() is False:
            self.jacquard_tester.start()
            self.__appText('start singal moniter...', 3)
    ...
    def __start_button(self):
        ...
        if text == "start":
            ...
            ThreadHelper.stop_thread(self.current_test_thread)
            # print self.current_test_thread.is_alive()
            # print 'stop thread'
            self.jacquard_tester.stop_test()
            self._set_dut1_result(None, None, False)
            self._set_dut2_result(None, None, False)

    def __reset_button(self):
        if self.jacquard_tester.get_testing() is True:
            reply = QMessageBox.question(self, 'question', 'Testing in progress. Are you sure you want to reset?', QMessageBox.Yes | QMessageBox.No)
            if reply == QMessageBox.Yes:
                self.jacquard_tester.stop_test()   # 停止测试
                self.jacquard_tester.reset()
            else:
                return
        else:
