...
import queue
from testui3 import TestUI
from PLineView import PLineView
...
# 2019-04-10 V1.0.0.9---liyunhe
# 修改KS和KY第一段结束N为0.1N
# 修改振动加速器数据采集算法，提高采集频率

#2019-04-10 V1.0.1.0---liyunhe
#修改MCU连接程序部分

#2019-05-03 V1.0.1.1---daisunqian
#新增baseline_row_count 测试函数，通过传入配置文件的方式选择不同的配置文件，
# 修改了加载背景图片的路径

#2019-05-10 V1.0.1.2---daisunqian
#在贺工的帮助下修改了获取图片的类，并可以通过编译成exe文件执行

#2019-05-23 V1.0.1.3---daisunqian
#可以根据不同的IQC IPQC OQC 传入不同的limit

#2019-05-23 V1.0.1.4---liyunhe
#修改登陆UI 把IQC OQC IPQC 在ui中可以选择，集成自动识别MCU 串口

#2019-05-23 V1.0.1.5---liyunhe
#可视化数据，把数据通过图行的方式时时显示在UI上
...
class JacquardMainWindow(TestUI):
    appendInfo = pyqtSignal(str, int)  # 修改信息

    def __init__(self, config_name, limit_name):
        super(JacquardMainWindow, self).__init__()
        self.setWindowFlags(Qt.FramelessWindowHint)
        self._quen = queue.Queue()
        self.__back_touch()
        # 初始化配置文件对象
        ...
        self.appconfig = APPConfigparse(config_name)
        #self.limitConfig = LimitConfig(r'./limit.ini')
        self.limitConfig = LimitConfig(limit_name)
        # 自定义信号连接
        ...
        self.top_text.setText("{0}.{1}".format(APP_NAME,APP_VERSION))
        # 状态栏初始化函数
        self.init_statusbar()
        ...
        if self.jacquard_tester is None:
            ...
            self.jacquard_tester.manual_window2.connect(self.__manual_window2)
            self.jacquard_tester.display_msg.connect(self.__display_msg)
            self.jacquard_tester.touch_diffenrence.connect(self._touch_diffenrence)
    ...
    def _init_ui(self):
        ...
        self._PLWidget = PLineView()
        self.grid_touchview.addWidget(self._PLWidget, 0, 0)
        ...
        icon = QIcon()
        icon.addPixmap(QPixmap(Pictures.closefilename), QIcon.Normal, QIcon.Off)
        self.btn_exit.setIcon(icon)
        self.btn_exit.setStyleSheet(Stylesheets.btn_style_close)

        icon = QIcon()
        icon.addPixmap(QPixmap(Pictures.startfilename), QIcon.Normal, QIcon.Off)
        self.start.setIcon(icon)
        self.start.setStyleSheet(Stylesheets.btn_style_Cyan)

        icon = QIcon()
        icon.addPixmap(QPixmap(Pictures.resetfilename), QIcon.Normal, QIcon.Off)
    ...
    def __test_start(self):
        ...
        icon = QIcon()
        icon.addPixmap(QPixmap(Pictures.stopfilename), QIcon.Normal, QIcon.Off)
        self.start.setIcon(icon)
    ...
    # 采集touch difference
    def _touch_diffenrence(self, rdstr):
        # print '_touch_diffenrence', rdstr
        self._quen.put(rdstr)

    def __back_touch(self):
        def touch_th():
            last_str = ''
            while True:
                if not self._quen.empty():
                    datastr = last_str + self._quen.get()
                    if len(datastr) == 0:
                        time.sleep(0.01)
                        continue
                    last_str = ''
                    ...
                    for record in data_spls:
                        if len(record) > 1 and len(record.split(',')) >= 12:
                            try:
                                ls = [int(data1) for data1 in record.split(',')[1:]]  # 第1个是prox数据，从第二个开始
                                if len(ls) == 12:
                                    # print "ls",ls
                                    self._PLWidget.update_line(ls)
                                else:
                                    print "record=",record
                                    last_str = record
                            except Exception, ex:
        ...
        th = Thread(target=touch_th)
        th.setDaemon(True)
        th.start()

    def __manual_window(self, title, info,):
        from Comm.IndicationWindow import IndicationWindow
        IndicationWindow.WAIT_TIME_OUT = self.appconfig.wait_time_out
        self.jacquard_tester.open_Indication_window(title, info, Pictures.no_imgfilename, self)
    def __manual_window2(self):
        self.jacquard_tester.open_new_window()
    # 信息
    ...
    def __start_button(self):
        ...
        th = threading.Thread(target=self.jacquard_tester.start_test)
        th.start()
        return True
...
if __name__ == "__main__":
    import sys
    import serial
    from Controller.MCUComm import MCUComm
    from LoginselectView import LoginselectView
    app = QApplication(sys.argv)
    print sys.argv[1]
    print sys.argv[2]
    # loginwin = Login(sys.argv[1])
    loginwin = LoginselectView()
    isok = loginwin.exec_()
    cofig = "./IQCconfig.ini"
    limit = "./IQClimit.ini"
    if isok == 1:
        if loginwin.ipqc_test:
            cofig = "./IPQCconfig.ini"
            limit = "./IPQClimit.ini"
        elif loginwin.oqc_test:
            cofig = "./OQCconfig.ini"
            limit = "./OQClimit.ini"
        # ui = JacquardMainWindow(sys.argv[1],sys.argv[2])

        prots = _get_ports()
        if len(prots) == 1:
            appconfig = APPConfigparse(cofig)
            appconfig.MCU_Port = prots[0]
        elif len(prots) > 1:
            appconfig = APPConfigparse(cofig)
            chk = False
            try:
                if prots.index(appconfig.MCU_Port) >= 0:
                    ...
                    print data
                    if len(data) > 0:
                        appconfig.MCU_Port = appconfig.MCU_Port
                    else:
                        chk = True
                    mcu.close_up()
                    mcu.ClosePort()
        ...
        ui = JacquardMainWindow(cofig, limit)
        ui.maxwindow()
        ui.show()
        sys.exit(app.exec_())