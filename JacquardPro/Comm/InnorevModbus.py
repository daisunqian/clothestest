...
class InnorevModbus(object):
    ...
    def print_exception_text(self, value):
        ...
        if value == 3:
            exception_text = "Illegal data address "+str(value)
        if value == 4:
            exception_text = "Subordinate devices failure"
        if value == 5:
            exception_text = "Command acknowledge"
        if value == 6:
            exception_text = "Subordinate device busy"
        if value == 8:
    ...
    def InnorevOpenSerial(self):
        ...
        self.lock.acquire()
        try:             
            self.main.set_timeout(0.1)
            self.main.set_verbose(True)
            return True
    ...
    def InnorevReadCoil(self,subordinateaddress,rigster,data_number = 1):
        """
        ...
        data = None
        try:
            data = self.main.execute(subordinateaddress, cst.READ_COILS, rigster, data_number)
            self.lock.release()
            return data
    ...
    def InnorevReadDiscreteInput(self,subordinateaddress,rigster,data_number = 1):
        """
        ...
        data = None
        try:
            data = self.main.execute(subordinateaddress, cst.READ_DISCRETE_INPUTS, rigster, data_number)
            self.lock.release()
            return data
        except modbus_tk.exceptions.ModbusError as exc:  
            logger.error("%s", self.print_exception_text(exc.get_exception_code()))
    ...
    def InnorevReadHodlingRegister(self,subordinateaddress,rigster,data_number = 1,out_format=""):
        """
        ...
        data = None
        try:
            data = self.main.execute(subordinateaddress, cst.READ_HOLDING_REGISTERS, rigster, data_number,0,out_format)
            return data
        except modbus_tk.exceptions.ModbusError as exc:  
            logger.error("%s", self.print_exception_text(exc.get_exception_code()))
            data = self.main.execute(subordinateaddress, cst.READ_HOLDING_REGISTERS, rigster, data_number,out_format)
            return data
        except Exception as excpt:            
            logger.error(excpt)
            data = self.main.execute(subordinateaddress, cst.READ_HOLDING_REGISTERS, rigster, data_number,out_format)
    ...
    def InnorevWriteSignalCoil(self,subordinateaddress,rigster,data):
        """
        ...
        try:
            if(0 == data or 1 == data):
                self.main.execute(subordinateaddress, cst.WRITE_SINGLE_COIL, rigster, output_value=data)
            ...
            self.lock.release()    
    def InnorevWriteSingleRegister(self,subordinateaddress,rigster,data):
        """
        ...
        self.lock.acquire()
        try:
            da = self.main.execute(subordinateaddress, cst.WRITE_SINGLE_REGISTER, rigster, output_value=data)
            return True
        except modbus_tk.exceptions.ModbusError as exc:  
            logger.error("%s=====%s====%s", self.print_exception_text(exc.get_exception_code()),str(rigster),str(data))
            self.main.execute(subordinateaddress, cst.WRITE_SINGLE_REGISTER, rigster, output_value=data)
            return True
        except Exception as excpt:            
            logger.error(excpt)
            self.main.execute(subordinateaddress, cst.WRITE_SINGLE_REGISTER, rigster, output_value=data)
            ...
            self.lock.release()    
    def InnorevWriteMultipleCoils(self,subordinateaddress,rigster,data):
        """
        ...
        try:
            ...
            for length in range(0,tuple_len):
                if(not ((0 == tuple_name[length]) or (1 == tuple_name[length]))):
                    ...
                    Islegal = False
            if(Islegal):
                self.main.execute(subordinateaddress, cst.WRITE_MULTIPLE_COILS, rigster, output_value = tuple_name)
            ...
            self.lock.release()
    def InnorevWriteMultipleRegister(self,subordinateaddress,rigster,data):
        """
        write multiple 0/1 into coils
        if return False,some error may happen   
        """
        ...
        try:
            tuple_name = tuple(data)           
            self.main.execute(subordinateaddress, cst.WRITE_MULTIPLE_REGISTERS, rigster, output_value = tuple_name)
